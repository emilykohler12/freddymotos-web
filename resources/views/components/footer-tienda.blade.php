            ...
            <div class="col-span-2 lg:col-span-1">
                ...
                <div class="mt-5 flex gap-3">
                    @if ($settings->instagram_url)
                        <a href="{{ $settings->instagram_url }}" target="_blank" rel="noopener" aria-label="Instagram" class="flex h-9 w-9 items-center justify-center rounded-full bg-marca-negro/10 text-marca-negro transition hover:bg-marca-negro hover:text-marca-blanco">
                    ...
                    @if ($settings->facebook_url)
                        <a href="{{ $settings->facebook_url }}" target="_blank" rel="noopener" aria-label="Facebook" class="flex h-9 w-9 items-center justify-center rounded-full bg-marca-negro/10 text-marca-negro transition hover:bg-marca-negro hover:text-marca-blanco">
                    ...
                    @if ($settings->whatsapp_link)
                        <a href="{{ $settings->whatsapp_link }}" target="_blank" rel="noopener" aria-label="WhatsApp" class="flex h-9 w-9 items-center justify-center rounded-full bg-marca-negro/10 text-marca-negro transition hover:bg-marca-negro hover:text-marca-blanco">
                ...
                <ul class="mt-4 space-y-3 text-sm text-marca-negro/70">
                    @if ($settings->telefono)
                        <li class="flex items-start gap-2">
                            <svg class="mt-0.5 h-4 w-4 shrink-0 text-marca-negro" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M3 5l4-1 2 5-3 2a12 12 0 006 6l2-3 5 2-1 4a2 2 0 01-2 2A16 16 0 013 7a2 2 0 010-2z"/></svg>
                            <a href="{{ $settings->tel_link }}" class="transition hover:text-marca-rojo">{{ $settings->telefono }}</a>
                        </li>
                    @endif
                    @if ($settings->direccion)
                        <li class="flex items-start gap-2">
                            <svg class="mt-0.5 h-4 w-4 shrink-0 text-marca-negro" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M12 21s7-5.5 7-11a7 7 0 10-14 0c0 5.5 7 11 7 11zm0-8a2.5 2.5 0 100-5 2.5 2.5 0 000 5z"/></svg>
                            <span>{{ $settings->direccion }}</span>
                        </li>
                    @endif
                    @if ($settings->email)
                        <li class="flex items-start gap-2">
                            <svg class="mt-0.5 h-4 w-4 shrink-0 text-marca-negro" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M3 6h18v12H3zM3 7l9 6 9-6"/></svg>
                            <a href="mailto:{{ $settings->email }}" class="transition hover:text-marca-rojo">{{ $settings->email }}</a>
                        </li>
                    @endif
                    @if ($settings->horario_atencion)
                        <li class="flex items-start gap-2">
                            <svg class="mt-0.5 h-4 w-4 shrink-0 text-marca-negro" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 2m6-2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
