...
@section('content')
    ...
    <div class="max-w-2xl">
        ...
        {{-- Crear --}}
        <form method="POST" action="{{ route('admin.categories.store') }}" class="mb-6 flex gap-3 rounded-2xl bg-marca-blanco p-5 shadow-sm ring-1 ring-marca-gris-oscuro/5">
            @csrf
        ...
        <div class="rounded-2xl bg-marca-blanco shadow-sm ring-1 ring-marca-gris-oscuro/5">
            @if ($categories->isEmpty())
                <p class="px-5 py-10 text-center text-sm text-marca-gris-oscuro">Todavía no hay categorías.</p>
            @else
                            ...
                            <details class="group">
                                <summary class="flex cursor-pointer list-none items-center justify-between gap-3">
                                    ...
                                    <span class="flex items-center gap-3">
                                        ...
                                        <svg class="h-4 w-4 text-marca-gris-oscuro transition group-open:rotate-180" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                ...
                                <div class="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center">
                                    <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="flex flex-1 gap-2">
                                        ...
                                        <button type="submit" class="shrink-0 rounded-lg border border-marca-gris-oscuro/20 px-4 py-2 text-xs font-semibold text-marca-negro transition hover:border-marca-amarillo">
                                            Guardar
                                        </button>
                                    </form>
                                    <form method="POST" action="{{ route('admin.categories.destroy', $category) }}" onsubmit="return confirm('¿Eliminar la categoría {{ $category->name }}?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="shrink-0 rounded-lg border border-marca-rojo/30 px-4 py-2 text-xs font-semibold text-marca-rojo transition hover:bg-marca-rojo hover:text-marca-blanco">
                                            Eliminar
                                        </button>
                                    </form>
                                </div>
